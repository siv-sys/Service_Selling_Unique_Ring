        ...
        >
          ...
          <div className="sticky top-0 bg-white dark:bg-charcoal border-b border-primary/10 p-6 flex justify-between items-center z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="heading-serif text-2xl font-semibold text-pink-900 dark:text-white">Purchase History</h2>
                {orders.length > 0 && (
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center justify-center transition-colors text-slate-500 dark:text-slate-400"
            >
          ...
          {/* Content */}
          <div className="p-6 bg-pink-200 dark:bg-slate-800/50 rounded-b-2xl">
            {loading ? (
              ...
              <div className="text-center py-12">
                <span className="material-symbols-outlined text-6xl text-slate-300 dark:text-slate-600 mb-4">receipt</span>
                <h3 className="text-xl font-bold text-pink-900 dark:text-white mb-2">No purchase history yet</h3>
                <p className="text-slate-500 dark:text-slate-900 mb-6">Your completed purchases will appear here</p>
                <button
                  onClick={onClose}
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-primary/10">
                  ...
                  <div className="border-b border-primary/10 pb-4 mb-4">
                    <div className="flex justify-between items-start flex-wrap gap-2 mb-2">
                      <div>
                        <h3 className="text-xl font-bold text-pink-900 dark:text-white">Receipt #{selectedOrder.order_number}</h3>
                        <p className="text-sm text-slate-500 dark:text-slate-900">{formatDate(selectedOrder.created_at)}</p>
                      </div>
                  ...
                  <div className="mb-6 p-4 bg-primary/5 rounded-xl">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                      <p><span className="text-slate-900">Name:</span> <span className="text-slate-900 dark:text-white">{selectedOrder.shipping_name || selectedOrder.user?.full_name || 'N/A'}</span></p>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="space-y-3 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl">
                      {selectedOrder.items?.map((item, idx) => (
                        <div key={idx} className="flex gap-4 p-3 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700 bg-white-50">
                          <div className="w-16 h-16 rounded-lg overflow-hidden bg-slate-100 dark:bg-slate-800 flex-shrink-0">
                            <img src={item.image_url} alt={item.ring_name} className="w-full h-full object-cover" onError={(e) => {
                              (e.target as HTMLImageElement).src = 'https://via.placeholder.com/64?text=Ring';
                            }} />
                          </div>
                          <div className="flex-1">
                            <p className="font-bold text-slate-900 dark:text-white">{item.ring_name}</p>
                  ...
                  <div className="border-t border-primary/10 pt-4">
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between text-slate-900">
                        ...
                        <span className="text-slate-900 dark:text-white">${selectedOrder.subtotal.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-slate-900">
                        <span className="text-slate-900">Shipping:</span>
                        <span className="text-slate-900 dark:text-white">${selectedOrder.shipping_cost}</span>
                      </div>
                      <div className="flex justify-between text-slate-900">
                      ...
                      <div className="flex justify-between pt-2 border-t border-primary/10">
                        ...
                        <span className="text-2xl font-bold text-yellow-500">${selectedOrder.total.toLocaleString()}</span>
                      </div>
                    </div>
                    <p className="text-xs text-slate-400 mt-4 flex items-center gap-1 text-pink-900 dark:text-pink-900">
              ...
              <div className="space-y-4">
                {orders.map((order) => (
                  <div
                    ...
                    className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-primary/10 hover:border-primary/30 cursor-pointer transition-all"
                  >
                    <div className="flex justify-between items-start mb-3 flex-wrap gap-2">
                      <div>
                        <p className="font-bold text-slate-900 dark:text-white">Order #{order.order_number}</p>
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex -space-x-2">
                        {order.items?.slice(0, 3).map((item, idx) => (
                          <div key={idx} className="w-8 h-8 rounded-full overflow-hidden border-2 border-white dark:border-slate-800">
                            <img src={item.image_url} alt={item.ring_name} className="w-full h-full object-cover" onError={(e) => {
                              (e.target as HTMLImageElement).src = 'https://via.placeholder.com/32?text=Ring';
                            }} />
                          </div>
                        ))}
